    ...
    >
      {open ? (
        <motion.button
          ...
          className='fixed inset-0 z-50 cursor-pointer bg-surface/80 backdrop-blur-sm md:hidden'
          exit={{ opacity: 0 }}
          initial={{ opacity: 0 }}
      ...
      {open ? (
        <motion.div
          ...
          aria-modal='true'
          className='fixed top-0 right-0 bottom-0 z-60 flex w-[80%] max-w-sm flex-col border-l border-white/5 bg-surface-secondary p-10 md:hidden'
          exit={{ x: '100%' }}
          initial={{ x: '100%' }}
            ...
            {isAdmin && (
              <Link
                className={cn(
                  ...
                  pathname === adminHref ? 'text-brand' : 'text-content-primary'
                )}
                href={adminHref}
                onClick={onClose}
              >
